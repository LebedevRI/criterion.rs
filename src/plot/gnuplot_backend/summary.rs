...
fn compute_range(
    ...
    match axis_range {
        AxisRange::Fit => {
            let i = Lerp::new(apply_axis_scale(*v.start()), apply_axis_scale(*v.end()));
            Range::Limits(
                deapply_axis_scale(i.eval(0.0 - offset)),
                deapply_axis_scale(i.eval(1.0 + offset)),
            )
        }
        AxisRange::FullScale => {
            let i = Lerp::new(0., apply_axis_scale(*v.end()));
            Range::Limits(0.0, deapply_axis_scale(i.eval(1.0 + offset)))
        }
